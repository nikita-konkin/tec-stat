...
from app.models.schemas import AvailabilityResponse, StatisticsResponseCB, TimeSeriesPointCB

router = APIRouter(prefix="/cb", tags=["CB", "Data Analysis"])
...
def list_stations(
    ...
    """Discover which stations have CB data for a given year/day (same as AbsolTEC availability)."""
    ...
    return format_payload(payload, format, f"cb_stations_{year}_{doy:03d}")
...
def list_days(
    ...
    """List all days-of-year for which a station has CB data in a given year."""
    root = settings.get_absoltec_root(data_root)
    payload = AvailabilityResponse(year=year, station=station,
                                   days=absoltec_discover_days(root, year, station))
    return format_payload(payload, format, f"cb_days_{year}_{station.lower()}")


@router.get("/raw", response_model=list[TimeSeriesPointCB])
def raw_data(
    year:    int = Query(..., ge=2000, le=2100),
    doy:     int = Query(..., ge=1,    le=366),
    station: str = Query(..., min_length=2, max_length=9),
    ...
    """Raw 48-point CB time series for one station/day."""
    payload = get_raw_data_cb(year, doy, station, settings.get_absoltec_root(data_root))
    return format_payload(payload, format, f"cb_raw_{year}_{doy:03d}_{station.lower()}")
...
def raw_data_range(
    ...
    return format_payload(
        payload,
        format,
        f"cb_raw_range_{year}_{doy_start:03d}_{doy_end:03d}_{filename_stations}",
    )


@router.get("/statistics", response_model=StatisticsResponseCB)
def statistics(
    year:      int   = Query(..., ge=2000, le=2100),
    doy_start: int   = Query(..., ge=1,    le=366),
    doy_end:   int   = Query(..., ge=1,    le=366),
    ...
    """Mean ± Student-CI for CB values of a station over a day range."""
    if doy_start > doy_end:
        raise HTTPException(422, "doy_start must be ≤ doy_end")
    payload = compute_statistics_cb(year, doy_start, doy_end, station, alpha,
                                    settings.get_absoltec_root(data_root))
    return format_payload(
        payload,
        format,
        f"cb_stats_{year}_{doy_start:03d}_{doy_end:03d}_{station.lower()}",
    )
...
def statistics_per_station_day(
    ...
    if doy_start > doy_end:
        raise HTTPException(422, "doy_start must be ≤ doy_end")
    payload = compute_statistics_per_station_day_cb(
        year, doy_start, doy_end, stations, alpha,
        settings.get_absoltec_root(data_root))
    return format_payload(
        payload,
        format,
        f"cb_stats_per_station_day_{year}_{doy_start:03d}_{doy_end:03d}",
    )